	...
	<div class="newsDetail">
		...
		<div class="bread"  v-show="showPage">
			<ul class="clear">
				...
				<li><router-link to="/news">{{lang==0?'新闻中心':'NEWS'}}</router-link></li>
	...
	export default{
    	...
    	methods:{
			...
			getData(){
				...
				this.$http.jsonp(newsUrl+'News_Interface_GetContent.html?lang='+lang+'&pid=1&field=13-15-18-19-122-123-124&method=s&nid='+this.$route.params.id+'&sort=1&neighbor=2&nfield=13').then((response) => {
					/*&field=13-15-18-19*/
					this.newsDetail=response.body.content;
					/*that.$emit('title','title');*/
					that.title = response.body.content.SEO_Title;
					that.description = response.body.content.SEO_Description;
					that.keywords = response.body.content.SEO_Keywords;
