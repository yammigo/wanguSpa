	...
	<div class="news">
		...
		<div v-show="showPage" class="newsCon clear">
			<div class="item" v-for="(item,index) in news" v-show =  'index > (page-1)*9 && index <= (page * 9) '>
				 <div class="img">
				 	<router-link :to="{ path: 'disDetail/'+item.NewsId}" class="bgImg" :style="{backgroundImage:'url(\''+item.ThumbPicture+'\')'}"></router-link>
				 </div>
				 <!-- :style="{backgroundImage:'url(\''+ bannerUrl + 'public/' +item.SlideImage+'\')'}" -->
				 <span class="line"></span>
				 <router-link class="name" :to="{ path: 'disDetail/'+item.NewsId}">{{item.NewsTitle}}</router-link>
				 <div class="text" v-html="item.NewsContentShort"></div>
				 <div class="bottom clear">
				 	<p>
				 		<span><i class="iconfont icon-time"></i></span>{{item.Month}}/{{item.Day}}  {{item.Year}}
				 	</p>
				 	<router-link class="more fr" :to="{ path: 'disDetail/'+item.NewsId}">{{lang==0?'查看详情':'Details'}} <span class="icon"></span></router-link>
				 </div>
			</div>
		</div>
